...
void FluidIntersection() {
    ...
    for(uint step=0;step<27&&t<farT;++step) {
        ...
        if(endT>t&&(mask&(1u<<(cellId%32)))) {
            ...
            uint visibilityFlags=RAY_FLAG_FORCE_OPAQUE|RAY_FLAG_ACCEPT_FIRST_HIT_AND_END_SEARCH|RAY_FLAG_SKIP_CLOSEST_HIT_SHADER;
            bool visibilityOnly=(RayFlags()&visibilityFlags)==visibilityFlags && !(OpticalControls.w&2048u);
            float r;if(liquidRoot(c,r,visibilityOnly)){FluidAttributes a;a.unused=0;ReportHit(lerp(t,endT,r),0,a);return;}
        }
